...
/*   Updated: 2026/01/09 01:20:02 by lilypad          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*lexer_single token*/
t_token_type				scan_next_token(char *input, size_t *len,
								t_shell_context *sh);
t_token_type				token_subshell(char *input, size_t *len);
t_token_type				token_and_or_pipe(const char *input, char c,
								size_t *len, t_shell_context *sh);
t_token_type				token_redir(char *input, char c, size_t *len,
								t_shell_context *sh);
t_token_type				token_word(char *input, size_t *len,
								t_shell_context *sh);
...
t_ast						*parse_pipeline(t_list **token,
								t_shell_context *sh);
t_ast						*parse_subshell(t_list **token,
								t_shell_context *sh);
...
bool						parse_new_command_arg(t_list **token,
								t_ast *command, t_shell_context *sh);
...
/*parser_redir_next*/
t_ast						*build_redirected_command(t_ast *prefix,
								t_ast *suffix, t_ast *command);
t_ast						*get_last_redirection_list(t_ast *node);

/*parser_build_node*/
t_ast						*create_ast_command(char **argv,
								t_shell_context *sh);
t_ast						*create_ast_subshell(t_ast *child,
								t_shell_context *sh);
t_ast						*create_ast_redir(t_token_type direction,
								t_list *tk_filename, t_ast *child,
								t_shell_context *sh);
t_ast						*create_ast_pipeline(t_ast *left, t_ast *right,
								t_shell_context *sh);
t_ast						*create_ast_and_or(t_ast *left,
								t_token_type operator, t_ast * right,
								t_shell_context *sh);
...
void						lst_add_back_s(void *content, t_list **lst,
								t_tracking_scope scope, t_shell_context *sh);
void						check_node_alloc(t_list *new_node, void *pointer,
								t_shell_context *sh);
...
void						add_arg_to_array(char ***array, char *new_arg,
								t_shell_context *sh);
...
void						*calloc_s(size_t count, size_t size,
								t_tracking_scope scope, t_shell_context *sh);
void						track_alloc(void *pointer, t_tracking_scope scope,
								t_shell_context *sh);
