...
*/
int	execute_command(t_ast_command *cmd, t_exec_context exectuion_context,
		t_shell_context *shell_conetext)
{
	...
	if (!cmd->args || !cmd->args[0])
		return (EXIT_SUCCESS);
	isbuiltin = is_buildtin(cmd->args[0]);
		...
		t_shell_context *shell_conetext)
{
	...
	report_child_termination_signal(wait_status, NULL, shell_conetext);
